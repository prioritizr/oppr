...
simulate_ppp_data <- function(number_features, cost_mean = 100, cost_sd = 5,
  ...
  if (locked_in_proportion > 1e-10) {
    l <- sample.int(
      number_features, ceiling(number_features * locked_in_proportion)
    )
  ...
  projects <- tibble::tibble(
    name = c(
      paste0("F", seq_len(number_features), "_project"),
      "baseline_project"
    ),
    success = c(
      stats::runif(
        number_features, success_min_probability, success_max_probability
      ),
      1
    )
  )
  ...
  diag(spp_prob_matrix) <- stats::runif(
    number_features,
    funded_min_persistence_probability,
