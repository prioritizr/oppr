...
#' each feature will persist if it is funded and successfully
#' completed. Missing values (`NA`)
#' indicate that a feature does not benefit from a project being funded.
#' }
#'
...
simulate_ptm_data <- function(number_projects, number_actions, number_features,
  ...
  actions <- tibble::tibble(
    name = c(
      paste0("action_", seq_len(number_actions)),
      "baseline_action"
    ),
  ...
  projects <- tibble::tibble(
    ...
    success = c(
      stats::runif(
        number_projects, success_min_probability, success_max_probability
      ),
      1
    )
  )
  ...
  spp_prob_matrix <- matrix(
    stats::runif(
      number_features * (number_projects + 1),
      funded_min_persistence_probability,
  ...
  for (i in seq_len(nrow(spp_prob_matrix) - 1)) {
    spp_prob_matrix[
      i,
      sample(seq_len(number_features), sample.int(number_features - 1, 1))
    ] <- NA_real_
  }
  ## assign probabilities for baseline project
  spp_prob_matrix[nrow(spp_prob_matrix), ] <- stats::runif(
    number_features,
    baseline_min_persistence_probability,
    baseline_max_persistence_probability
  )
  ...
  for (i in seq_len(number_projects)) {
    organization_data[
      i,
      sample(seq_len(number_actions), sample.int(number_actions, 1))
    ] <- TRUE
  }
