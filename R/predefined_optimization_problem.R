...
#'
#' @details
#' The argument to `x` must be a list that contains the following
#' elements.
#'
#' \describe{
#'
...
#' l <- list(
#'   modelsense = "min", number_of_projects = 2,
...
predefined_optimization_problem <- function(x, data = list()) {
  assertthat::assert_that(
    ...
    is.finite(x$number_of_features),
    assertthat::is.count(x$number_of_branches),
    is.finite(x$number_of_branches),
    assertthat::is.count(x$number_of_projects),
    is.finite(x$number_of_projects),
    assertthat::is.count(x$number_of_actions),
    is.finite(x$number_of_actions),
    is.numeric(x$obj),
    all(is.finite(x$obj)),
    is.numeric(x$lb),
    all(is.finite(x$lb)),
