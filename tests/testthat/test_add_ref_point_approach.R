test_that("sum method", {
  ...
  s1 <-
    p %>%
    add_ref_point_approach(weights = c(1, 0), goals = c(200, 2), "sum") %>%
    solve()
  s2 <-
    p %>%
    add_ref_point_approach(weights = c(0, 1), goals = c(200, 2), "sum") %>%
    solve()
  # run tests
  ## s1
...
test_that("max method", {
  ...
  s1 <-
    p %>%
    add_ref_point_approach(weights = c(1, 0), goals = c(200, 2), "max") %>%
    solve()
  s2 <-
    p %>%
    add_ref_point_approach(weights = c(0, 1), goals = c(200, 2), "max") %>%
    solve()
  s3 <-
    p %>%
    add_ref_point_approach(weights = c(0.5, 0.5), goals = c(200, 2), "max") %>%
    solve()
  # run tests
  ## s1
  expect_equal(s1$A1, TRUE)
