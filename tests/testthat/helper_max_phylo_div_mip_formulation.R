max_phylo_div_mip_formulation <- function(project_data, action_data, tree,
  ...
  ## if needed, set default locked values
  if (!assertthat::has_name(action_data, "locked_in")) {
    action_data$locked_in <- FALSE
  }
  if (!assertthat::has_name(action_data, "locked_out")) {
    action_data$locked_out <- FALSE
  }
  ...
  ## create matrix with feature vs. project information
  fp <- as.matrix(project_data[, species_names, drop = FALSE]) > 0
  fp[is.na(fp)] <- FALSE
  fp_s <- which(fp > 0.5, arr.ind = TRUE)[, 2]
  fp_j <- which(fp > 0.5, arr.ind = TRUE)[, 1]

  ## create variable names
  variable_names <- c(
    paste0("X_", seq_len(n_actions)),
    paste0("Y_", seq_len(n_projects)),
    paste0("Z_", fp_j, species_names[fp_s]),
  ...
  model$vtype[n_actions + n_projects + seq_len(sum(fp))] <- "B"
  ...
  for (s in species_names) {
    curr_row <- curr_row + 1
    for (j in seq_len(n_projects)) {
      if (isTRUE(project_data[[s]][j] > 1.0e-15)) {
        model$A[curr_row, paste0("Z_", j, s)] <- 1
      }
    }
    model$sense[curr_row] <- "="
  ...
  model$rownames[curr_row] <- "C6"

  ## drop zeros from constraint matrix
  model$A <- Matrix::drop0(model$A)
