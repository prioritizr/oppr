...
To start off, we will initialize the random number generator to ensure reproducibility. Next, we will load the _ppr R_ package. And then we will load the _ggplot2_, _tibble_, and _tidyr R_ packages to help with data for visualizations and wrangling.
...
We could also save the solution table to our computer (e.g. so we could view it in a spreadheeting program, such as Microsoft Excel).
...
# save table to file
write.table(s1, "results.csv" , sep = ",", row.names = FALSE)
```
