...
# rank projects using the backwards heuristic algorithm
s7_ranks <-
  problem(projects = projects, actions = actions, features = features,
              "name", "success", "name", "cost", "name") %>%
  add_max_richness_objective(budget = max(budgets)) %>%
  add_feature_weights("weight") %>%
  add_binary_decisions() %>%
  add_heuristic_solver(verbose = FALSE,
                       number_solutions = nrow(projects) + 1) %>%
  solve()

# find best solution for each budget increment
s7 <- lapply(budgets, function(x) {
  s <- s7_ranks[s7_ranks$cost <= x, , drop = FALSE]
  s <- s[which.max(s$obj), , drop = FALSE]
  s$budget <- x
  s
})
