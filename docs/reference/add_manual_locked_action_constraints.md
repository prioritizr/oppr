...
particular actions are selected, or not selected, for funding by the
...
- `"action"`:
  ...
  `character` values with action names.

- `"status"`:

  `numeric` values indicating if actions should be selected for funding
...
#> Presolve time: 0.00s
#> Presolved: 7 rows, 11 columns, 14 nonzeros
#> Variable types: 0 continuous, 11 integer (11 binary)
#> Root relaxation presolved: 7 rows, 11 columns, 14 nonzeros
#> 
#> 
...
#> 1        1 OPTIMAL  403.  2.91         0         1         1         1         1
