...
with the *SYMPHONY* software. This function can also be used to
customize the behavior of the solver. It requires the lpsymphony package
to be installed.
...
field that includes linear programming). The lpsymphony package is
distributed through
[Bioconductor](https://doi.org/doi:10.18129/B9.bioc.lpsymphony). This
functionality is provided because the lpsymphony package may be easier
to install to install on Windows and Mac OSX systems than the Rsymphony
package.
...
#> 1        1 TM_OPT…  195.  2.19 TRUE      TRUE      FALSE     FALSE     FALSE    
