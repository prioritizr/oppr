...
To start off, we will initialize the random number generator to ensure reproducibility. Next, we will load the _ppr R_ package. And then we will load _ggplot2_ and _tibble R_ packages to help with data for visualizations and wrangling.

```{r}
# set random number generated seed
set.seed(1000)
# load ppr package for project prioritization
library(ppr)
...
s8 <- lapply(budgets, function(b) {
  o <- problem(projects = projects, actions = actions, features = features,
       ...
       add_feature_weights("weight") %>%
       add_binary_decisions() %>%
       add_random_solver(verbose = FALSE, number_solutions = 1000) %>%
       solve()
...
Here, we can see the that heuristic algorithm generally performs better than funding conservation projects using random processes, so you might think that this means that heuristic algorithms can perform pretty well on balance. But we can do better. Let's generate a series of solutions using exact algorithms with various budgets.
